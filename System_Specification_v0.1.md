...
1. **שליפת נתונים (Ingestion):** המערכת תבצע שליפה שבועית (Bulk) מה-API של הפורטל. כל רשומה תכלול את שדות הליבה וכן את שדה `StatusLastUpdateDate` (תאריך עדכון סטטוס אחרון), `FeedbackStatus`, `ErrorCodeV4Id`, השילוב של `KodKupa_IdentityNumber` ו־`KodKupa_IncomeTax`, הסכום המקורר ופרטי הלקוח (`CustomerNumber`, `MISPAR_MEZAHE_OVED`).
2. **סינון זמן וסטטוס:**
...
3. **חישוב משך תקלה (Duration Calculation):**
   - לכל רשומה נחשב במשך כמה שבועות היא נמצאת באותו סטטוס באמצעות `StatusLastUpdateDate` ומידע היסטורי. זהו המשך הרציף של מועדון השבועות (Unique Week Start) עד לשבוע הנוכחי.
   - הנתון אינו נחשף ישירות למעסיק, אך משמש לניתוח פנימי ולביצוע escalations / תזכורות.
4. **מיפוי שגיאות:** הצמדת הוראות טיפול לפי קוד שגיאה (`ErrorCodeV4Id`) בהתאם לקובץ `error_code_mapping_final.xlsx` (כולל תבניות נושא/גוף, גורם אחראי, מיילים קבועים).
...
1. **בדיקת מונה חזרות (Retry Logic):**
   ...
   - **מדיניות מיילים:** בכל שלב נוצר מייל חדש. בימים בהם Counter ≥ 2 יתווספו תמלילים בקול של תזכורת/דחיפות, ובמידה ו־Counter ≥ 3 יכללו רשימת נמענים מורחבת לפי החוקיות (עם חיבור ל־Salesforce לקבלת גורמי פיקוח).
...
1. **מערכת הליבה ("השקט שלך"):**
   ...
   - **API נדרש:** `GetHistoricalStatus` (או שאילתה דומה) עבור קודים 4,5,15,93 – לשליפת רשומות מהחודשים הקודמים שמוגדרות כקליטה תקינה.  
